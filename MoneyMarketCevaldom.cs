...
namespace WindowsService1
{
    public partial class MoneyMarketCevaldom : ServiceBase
    {
        ...
        public string strruta = @"\\ARP01\xml\";
                    ...
                    if (dsoperaciones.Tables[0].Rows.Count > 0)
                    {
                        try
                        {
                            while (i <= dsoperaciones.Tables[0].Rows.Count - 1)
                            {
                                ...
                                {
                                    //string url = "http://172.16.40.105:8080/cevaldom-webservices/negotiation/v1/operationRequest?SolOperaciones=";
                                    //string url = "http://prejbosrv02.cevaldom.local:8080/cevaldom-webservices/negotiation/v1/operationRequest?SolOperaciones=";
        ...
        public void DesBloqueoArchivo(string filename)
        {
            ...
            {
                if (p.MachineName.ToUpper() == "ADB03" || p.MachineName.ToUpper() == "SEP" || p.MachineName.ToUpper() == "ARP01" || p.MachineName.ToUpper() == ".")
